...
function SocketIoServer(zookeeper, socketio_port, kafka, io, metrics) {
	var _this = this
...
SocketIoServer.prototype.handleResendRequest = function(connection, req) {
	...
	if (!stream) {
		stream = this.createStreamObject(req.channel, req.canvas)
	}
	...
	var resendProtocolWrapper = function(first, last, resendFunc) {
		...
		_this.metrics && _this.metrics.increment("resend", stream.id, last - first + 1, stream.canvasId)
		resendFunc(function() {
 ...
 * end up in subscribed state within one minute.
 **/
SocketIoServer.prototype.createStreamObject = function(streamId, canvasId) {
	var _this = this
	var stream = new Stream(streamId, 'init', this.kafka)
	stream.id = streamId
	stream.canvasId = canvasId
...
SocketIoServer.prototype.getListenerCount = function(streamId) {
...
SocketIoServer.prototype.emitUiMessage = function(message, streamId) {
	var stream = this.streams[streamId]
	stream && this.metrics && this.metrics.increment("eventsOut", streamId, this.getListenerCount(streamId), stream.canvasId)
...
SocketIoServer.prototype.handleSubscribeRequest = function(connection, request) {
	...
	if (!request.channel) {
		...
		if (!stream) {
			stream = this.createStreamObject(request.channel, request.canvas)
		}
		...
		// If the Stream is not yet subscribed, wait for the event, otherwise call directly
		if (stream.state === 'subscribed') {
