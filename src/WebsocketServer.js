...
module.exports = class WebsocketServer extends events.EventEmitter {
    ...
    handlePublishRequest(connection, req) {
        ...
        this.streamFetcher.authenticate(req.stream, req.authKey, req.sessionToken, 'write')
    ...
    handleSubscribeRequest(connection, request) {
        ...
        if (!request.stream) {
            ...
            this.streamFetcher.authenticate(streamId, request.authKey, request.sessionToken)
                .then((/* streamJson */) => {
